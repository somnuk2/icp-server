                      ...
                      <q-table title="ข้อมูลปริญญา" :rows="individuals1" :columns="columns" row-key="degree_id"
                        ...
                        <template v-slot:body-cell-actions="props">
                          <q-td :props="props" class="text-center">
                            <q-btn size="sm" color="blue" label="แก้ไข" unelevated no-caps @click="editUser(props.row.degree_id)"></q-btn>
...
export default {
  ...
  data() {
    return {
      ...
      picked: new Date(),
      // ------------------------------------------------------------------------------
      title: "ข้อมูลระดับการศึกษา(ผู้ดูแลระบบ)",
  ...
  methods: {
    ...
    async exportTable() {
      ...
      try {
        ...
        mainTitle.value = `รายงานข้อมูลปริญญา (Admin Constance) - ${new Date().toLocaleDateString('th-TH')}`;
    ...
    async getFacultys() {
      if (!this.institute || !this.institute.value) {
        this.facultys.options = []; // Clear faculties if no institute is selected
