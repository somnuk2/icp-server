            ...
            <div class="col-md-8 offset-md-2 col-xs-12 q-pa-xs">
              <q-card flat class="bg-white text-black">
                      ...
                      <div class="row">
                        <div class="col-md-12 col-xs-12 q-pa-xs">
                          <q-input standout="bg-primary text-white" bottom-slots
                            v-model="individual.qualification_group_name" label="กลุ่มคุณสมบัติ/ทักษะ *" clearable :rules="[
                              (val) =>
                                (val && val.length > 0) || 'ต้องใส่ชื่อกลุ่ม',
                            ]">
                        ...
                        <div class="col-md-12 col-xs-12 q-pa-xs">
                          <q-input standout="bg-primary text-white" bottom-slots
                            v-model="individual.qualification_group_description" label="รายละเอียดกลุ่ม" clearable>
                            <template v-slot:prepend>
                ...
                <div class="row">
                  <div class="col-md-12 col-xs-12 q-pa-xs">
                    <div class="q-pa-xs">
                      <q-table title="กลุ่มคุณสมบัติ/ทักษะ" :rows="individuals1" :columns="columns" row-key="qualification_group_id"
                        ...
                        <template v-slot:top-right="props">
                          <div class="row q-gutter-sm items-center">
                            ...
                            <q-input dense debounce="300" v-model="filter" placeholder="ค้นหากลุ่มคุณสมบัติ..." outlined
                              bg-color="white">
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          name: "qualification_group_description",
          align: "left",
          label: "รายละเอียด",
          field: "qualification_group_description",
          sortable: true,
        },
      ],
      filter: ref(""),
  ...
  methods: {
    async exportTable() {
      ...
      try {
        ...
        const worksheet = workbook.addWorksheet('QualificationGroups');
        ...
        mainTitle.value = `รายงานข้อมูลกลุ่มคุณสมบัติ (Admin Constance) - ${new Date().toLocaleDateString('th-TH')}`;
        mainTitle.font = { name: 'Sarabun', size: 16, bold: true };
        mainTitle.alignment = { horizontal: 'center', vertical: 'middle' };
        worksheet.getRow(1).height = 40;

        const headerRow = worksheet.getRow(2);
        headerRow.values = ['รหัส', 'กลุ่มคุณสมบัติ/ทักษะ', 'รายละเอียด'];
        headerRow.height = 30;
        ...
        rows.forEach((row, idx) => {
          const r = worksheet.addRow([
            row.qualification_group_id,
            row.qualification_group_name || '-',
            row.qualification_group_description || '-'
          ]);
        ...
        worksheet.columns = [{ width: 10 }, { width: 40 }, { width: 50 }];

        const buffer = await workbook.xlsx.writeBuffer();
        const filename = (this.file_export || "QualificationGroups_Report").replace(/\.xlsx$/i, '') + '.xlsx';
    ...
    resetForm() {
      ...
      this.individual.qualification_group_description = "";
    },
    async submitForm() {
      const data = {
        qualification_group_name: this.individual.qualification_group_name,
        qualification_group_description: this.individual.qualification_group_description,
      };
      ...
      this.$q.dialog({
        ...
        try {
          if (!this.isEdit) {
            ...
            await axios.put(`${this.url_api}/${this.individual.qualification_group_id}`, data);
          }
    ...
    editUser(row) {
      ...
      this.isEdit = true;
      this.individual.qualification_group_id = row.qualification_group_id;
      this.individual.qualification_group_name = row.qualification_group_name;
      this.individual.qualification_group_description = row.qualification_group_description || "";
    },
    deleteUser(id, name) {
      this.$q.dialog({
        title: "ยืนยัน",
      ...
      this.$q.dialog({
        ...
        try {
          for (const item of this.selected) {
            try {
              await axios.delete(`${this.url_api}/${item.qualification_group_id}`);
              successCount++;
            } catch (err) {
              console.error(`Failed to delete ID ${item.qualification_group_id}:`, err);
              failCount++;
            }
          }
          this.$q.notify({
  ...
  created() {
    this.url_api = getRestApiUrl(this.$store) + "/qualification-groups";
  },
};
</script>
