                    ...
                    <q-form @submit.prevent="submitForm()" @reset="resetForm()" method="post" class="q-gutter-md">
                      ...
                      <div class="row">
                        <div class="col-md-9 col-xs-12 q-pa-xs">
                          <q-input standout="bg-primary text-white" bottom-slots v-model="individual.target_name"
                        ...
                        <div class="col-md-3 col-xs-12 q-pa-xs">
                          <q-input standout="bg-primary text-white" bottom-slots v-model="individual.target_value"
                      ...
                      <q-table title="ข้อมูลค่าเป้าหมาย" :rows="individuals1" :columns="columns" row-key="target_id"
                        ...
                        <template v-slot:top-right="props">
                          <div class="row q-gutter-sm items-center">
                            ...
                            <q-input dense debounce="300" v-model="filter" placeholder="ค้นหาเป้าหมาย..." outlined
                              bg-color="white">
                              <template v-slot:append>
                                <q-icon name="search" />
                              </template>
  ...
  methods: {
    async exportTable() {
      ...
      try {
        ...
        mainTitle.value = `รายงานข้อมูลค่าเป้าหมาย (Admin Constance) - ${new Date().toLocaleDateString('th-TH')}`;
        mainTitle.font = { name: 'Sarabun', size: 16, bold: true };
        mainTitle.alignment = { horizontal: 'center', vertical: 'middle' };
        worksheet.getRow(1).height = 40;

        const headerRow = worksheet.getRow(2);
        headerRow.values = ['รหัส', 'เป้าหมาย', 'ค่าเป้าหมาย'];
        headerRow.height = 30;
        ...
        const filename = (this.file_export || "Targets_Report").replace(/\.xlsx$/i, '') + '.xlsx';
      ...
      this.$q.dialog({
        ...
        try {
          for (const item of this.selected) {
            try {
              await axios.delete(`${this.url_api}/${item.target_id}`);
              successCount++;
            } catch (err) {
              console.error(`Failed to delete ID ${item.target_id}:`, err);
              failCount++;
            }
          }
          this.$q.notify({
