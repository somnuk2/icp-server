                      ...
                      <q-table title="ระดับความสำคัญ" :rows="individuals1" :columns="columns" row-key="level_id"
                        ...
                        <template v-slot:top-right="props">
                          <div class="row q-gutter-sm items-center">
                            ...
                            <q-input dense debounce="300" v-model="filter" placeholder="ค้นหารายการ..." outlined
                              bg-color="white">
                              <template v-slot:append>
                                <q-icon name="search" />
                              </template>
...
export default {
  ...
  methods: {
    async exportTable() {
      ...
      try {
        ...
        mainTitle.value = `รายงานข้อมูลระดับความสำคัญ (Admin Constance) - ${new Date().toLocaleDateString('th-TH')}`;
        mainTitle.font = { name: 'Sarabun', size: 16, bold: true };
        mainTitle.alignment = { horizontal: 'center', vertical: 'middle' };
        worksheet.getRow(1).height = 40;

        const headerRow = worksheet.getRow(2);
        headerRow.values = ['รหัส', 'ระดับความสำคัญ', 'ความหมาย', 'น้ำหนัก'];
        headerRow.height = 30;
        ...
        worksheet.columns = [{ width: 10 }, { width: 30 }, { width: 40 }, { width: 10 }];

        const buffer = await workbook.xlsx.writeBuffer();
        const filename = (this.file_export || "Levels_Report").replace(/\.xlsx$/i, '') + '.xlsx';
    ...
    resetForm() {
      ...
      this.individual.level_id = "";
      this.individual.level_name = "";
      this.individual.level_description = "";
      this.individual.weigth = "";
    },
    async submitForm() {
      const data = {
      ...
      this.$q.dialog({
        ...
        try {
          if (!this.isEdit) {
            ...
            await axios.put(`${this.url_api}/${this.individual.level_id}`, data);
          }
    ...
    editUser(row) {
      ...
      this.isEdit = true;
      this.individual.level_id = row.level_id;
      this.individual.level_name = row.level_name;
      this.individual.level_description = row.level_description;
      this.individual.weigth = row.weigth;
    },
    deleteUser(id, name) {
      this.$q.dialog({
        title: "ยืนยัน",
      ...
      this.$q.dialog({
        ...
        try {
          for (const item of this.selected) {
            try {
              await axios.delete(`${this.url_api}/${item.level_id}`);
              successCount++;
            } catch (err) {
              console.error(`Failed to delete ID ${item.level_id}:`, err);
              failCount++;
            }
          }
          this.$q.notify({
  ...
  created() {
    this.url_api = getRestApiUrl(this.$store) + "/level";
  },
};
</script>
