                      ...
                      <q-table title="กลุ่มอาชีพ" :rows="individuals1" :columns="columns" row-key="career_group_id" :filter="filter"
                        ...
                        <template v-slot:top-right="props">
                          <div class="row q-gutter-sm items-center">
                            ...
                            <q-input dense debounce="300" v-model="filter" placeholder="ค้นหากลุ่มอาชีพ..." outlined
                              bg-color="white">
...
export default {
  ...
  data() {
    return {
      ...
      isEdit: false,
      visibleColumns: ref(["actions", "career_group_id", "ca_group_name", "ca_group_description"]),
      columns: [
        ...
        {
          name: "ca_group_description",
          align: "left",
          label: "รายละเอียด",
          field: "ca_group_description",
          sortable: true,
        },
      ],
      filter: ref(""),
  ...
  methods: {
    async exportTable() {
      ...
      try {
        ...
        const worksheet = workbook.addWorksheet('CareerGroups');
        ...
        mainTitle.value = `รายงานข้อมูลกลุ่มอาชีพ (Admin Constance) - ${new Date().toLocaleDateString('th-TH')}`;
        mainTitle.font = { name: 'Sarabun', size: 16, bold: true };
        mainTitle.alignment = { horizontal: 'center', vertical: 'middle' };
        worksheet.getRow(1).height = 40;

        const headerRow = worksheet.getRow(2);
        headerRow.values = ['รหัส', 'กลุ่มอาชีพ', 'รายละเอียด'];
        headerRow.height = 30;
        ...
        const filename = (this.file_export || "CareerGroups_Report").replace(/\.xlsx$/i, '') + '.xlsx';
    ...
    resetForm() {
      ...
      this.individual.ca_group_description = "";
    },
    async submitForm() {
      const data = {
        ca_group_name: this.individual.ca_group_name,
        ca_group_description: this.individual.ca_group_description,
      };
      ...
      this.$q.dialog({
        ...
        try {
          if (!this.isEdit) {
            ...
            await axios.put(`${this.url_api}/${this.individual.career_group_id}`, data);
          }
    ...
    deleteSelected() {
      ...
      this.$q.dialog({
        ...
        message: `คุณต้องการลบกลุ่มอาชีพที่เลือกทั้งหมด ${this.selected.length} รายการหรือไม่?`,
        cancel: true,
        persistent: true,
      }).onOk(async () => {
        ...
        try {
          for (const item of this.selected) {
            try {
              await axios.delete(`${this.url_api}/${item.career_group_id}`);
              successCount++;
            } catch (err) {
              console.error(`Failed to delete ID ${item.career_group_id}:`, err);
              failCount++;
            }
          }
          this.$q.notify({
    ...
    editUser(row) {
      ...
      this.isEdit = true;
      this.individual.career_group_id = row.career_group_id;
      this.individual.ca_group_name = row.ca_group_name;
      this.individual.ca_group_description = row.ca_group_description || "";
    },
    deleteUser(id, name) {
      this.$q.dialog({
        title: "ยืนยัน",
  ...
  created() {
    this.url_api = getRestApiUrl(this.$store) + "/career_group";
  },
};
</script>
