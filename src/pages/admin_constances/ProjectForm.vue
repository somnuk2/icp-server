                      ...
                      <q-table title="โครงการ" :rows="individuals1" :columns="columns" row-key="project_id" :filter="filter"
                        ...
                        <template v-slot:top-right="props">
                          <div class="row q-gutter-sm items-center">
                            ...
                            <q-input dense debounce="300" v-model="filter" placeholder="ค้นหาโครงการ..." outlined
                              bg-color="white">
  ...
  methods: {
    async exportTable() {
      ...
      try {
        ...
        mainTitle.value = `รายงานข้อมูลโครงการ (Admin Constance) - ${new Date().toLocaleDateString('th-TH')}`;
        mainTitle.font = { name: 'Sarabun', size: 16, bold: true };
        mainTitle.alignment = { horizontal: 'center', vertical: 'middle' };
        worksheet.getRow(1).height = 40;

        const headerRow = worksheet.getRow(2);
        headerRow.values = ['รหัส', 'โครงการ', 'รายละเอียด'];
        headerRow.height = 30;
        ...
        const filename = (this.file_export || "Projects_Report").replace(/\.xlsx$/i, '') + '.xlsx';
      ...
      this.$q.dialog({
        ...
        try {
          for (const item of this.selected) {
            try {
              await axios.delete(`${this.url_api}/${item.project_id}`);
              successCount++;
            } catch (err) {
              console.error(`Failed to delete ID ${item.project_id}:`, err);
              failCount++;
            }
          }
          this.$q.notify({
