            ...
            <div class="col-md-8 offset-md-2 col-xs-12 q-pa-xs">
              <q-card flat class="bg-white text-black">
                      ...
                      <div class="row">
                        <div class="col-md-12 col-xs-12 q-pa-xs">
                          <q-input standout="bg-primary text-white" bottom-slots
                            v-model="individual.disability_name" label="ความพิการ *" clearable :rules="[
                              (val) =>
                        ...
                        <div class="col-md-12 col-xs-12 q-pa-xs">
                          <q-input standout="bg-primary text-white" bottom-slots
                            v-model="individual.disability_description" label="รายละเอียดความพิการ" clearable>
                            <template v-slot:prepend>
                ...
                <div class="row">
                  <div class="col-md-12 col-xs-12 q-pa-xs">
                    <div class="q-pa-xs">
                      <q-table title="ความพิการ" :rows="individuals1" :columns="columns" row-key="disability_id" :filter="filter"
                        ...
                        <template v-slot:top-right="props">
                          <div class="row q-gutter-sm items-center">
                            ...
                            <q-input dense debounce="300" v-model="filter" placeholder="ค้นหาความพิการ..." outlined
                              bg-color="white">
...
export default {
  ...
  data() {
    return {
      ...
      isEdit: false,
      visibleColumns: ref(["actions", "disability_id", "disability_name", "disability_description"]),
      columns: [
  ...
  methods: {
    async exportTable() {
      ...
      try {
        ...
        const worksheet = workbook.addWorksheet('Disabilities');
        ...
        mainTitle.value = `รายงานข้อมูลความพิการ (Admin Constance) - ${new Date().toLocaleDateString('th-TH')}`;
        mainTitle.font = { name: 'Sarabun', size: 16, bold: true };
        mainTitle.alignment = { horizontal: 'center', vertical: 'middle' };
        worksheet.getRow(1).height = 40;

        const headerRow = worksheet.getRow(2);
        headerRow.values = ['รหัส', 'ความพิการ', 'รายละเอียด'];
        headerRow.height = 30;
        ...
        const filename = (this.file_export || "Disabilities_Report").replace(/\.xlsx$/i, '') + '.xlsx';
    ...
    resetForm() {
      ...
      this.individual.disability_description = "";
    },
    async submitForm() {
      const data = {
        disability_name: this.individual.disability_name,
        disability_description: this.individual.disability_description,
      };
      ...
      this.$q.dialog({
        ...
        try {
          if (!this.isEdit) {
            ...
            await axios.put(`${this.url_api}/${this.individual.disability_id}`, data);
          }
    ...
    editUser(row) {
      ...
      this.isEdit = true;
      this.individual.disability_id = row.disability_id;
      this.individual.disability_name = row.disability_name;
      this.individual.disability_description = row.disability_description || "";
    },
    deleteUser(disability_id, disability_name) {
      this.$q.dialog({
        title: "ยืนยัน",
        message: `คุณต้องการลบ [ ${disability_id} - ${disability_name} ] หรือไม่ ?`,
        cancel: true,
        persistent: true,
      }).onOk(async () => {
        try {
          await axios.delete(`${this.url_api}/${disability_id}`);
      ...
      this.$q.dialog({
        ...
        try {
          for (const item of this.selected) {
            try {
              await axios.delete(`${this.url_api}/${item.disability_id}`);
              successCount++;
            } catch (err) {
              console.error(`Failed to delete ID ${item.disability_id}:`, err);
              failCount++;
            }
          }
          this.$q.notify({
  ...
  created() {
    this.url_api = getRestApiUrl(this.$store) + "/disabilities";
  },
};
</script>
