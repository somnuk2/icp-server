            ...
            <div class="col-md-8 offset-md-2 col-xs-12 q-pa-xs">
              <q-card flat class="bg-white text-black">
                      ...
                      <div class="row">
                        <div class="col-md-9 col-xs-12 q-pa-xs">
                          <q-input standout="bg-primary text-white" bottom-slots
                            v-model="individual.importance_name" label="ความสำคัญ *" clearable :rules="[
                              (val) =>
                        ...
                        <div class="col-md-3 col-xs-12 q-pa-xs">
                          <q-input standout="bg-primary text-white" bottom-slots
                            v-model="individual.importance_value" label="ค่าความสำคัญ *" clearable mask="#" fill-mask
                            hint="ค่าความสำคัญ:#" :rules="[
                              (val) =>
                                (val !== null && val !== '') ||
                                'ต้องใส่ค่าความสำคัญ',
                            ]">
                ...
                <div class="row">
                  <div class="col-md-12 col-xs-12 q-pa-xs">
                    <div class="q-pa-xs">
                      <q-table title="ค่าความสำคัญ" :rows="individuals1" :columns="columns" row-key="importance_id"
                        ...
                        <template v-slot:top-right="props">
                          <div class="row q-gutter-sm items-center">
                            ...
                            <q-input dense debounce="300" v-model="filter" placeholder="ค้นหาความสำคัญ..." outlined
                              bg-color="white">
                              <template v-slot:append>
                                <q-icon name="search" />
                              </template>
...
export default {
  ...
  methods: {
    async exportTable() {
      ...
      try {
        ...
        const worksheet = workbook.addWorksheet('Importances');
        ...
        mainTitle.value = `รายงานข้อมูลความสำคัญ (Admin Constance) - ${new Date().toLocaleDateString('th-TH')}`;
        mainTitle.font = { name: 'Sarabun', size: 16, bold: true };
        mainTitle.alignment = { horizontal: 'center', vertical: 'middle' };
        worksheet.getRow(1).height = 40;

        const headerRow = worksheet.getRow(2);
        headerRow.values = ['รหัส', 'ความสำคัญ', 'ค่าความสำคัญ'];
        headerRow.height = 30;
        ...
        worksheet.columns = [{ width: 10 }, { width: 40 }, { width: 20 }];

        const buffer = await workbook.xlsx.writeBuffer();
        const filename = (this.file_export || "Importances_Report").replace(/\.xlsx$/i, '') + '.xlsx';
    ...
    resetForm() {
      ...
      this.individual.importance_value = "";
    },
    async submitForm() {
      const data = {
      ...
      this.$q.dialog({
        ...
        try {
          if (!this.isEdit) {
            ...
            await axios.put(`${this.url_api}/${this.individual.importance_id}`, data);
          }
    ...
    editUser(row) {
      ...
      this.isEdit = true;
      this.individual.importance_id = row.importance_id;
      this.individual.importance_name = row.importance_name;
      this.individual.importance_value = row.importance_value;
    },
    deleteUser(id, name) {
      this.$q.dialog({
        title: "ยืนยัน",
      ...
      this.$q.dialog({
        ...
        try {
          for (const item of this.selected) {
            try {
              await axios.delete(`${this.url_api}/${item.importance_id}`);
              successCount++;
            } catch (err) {
              console.error(`Failed to delete ID ${item.importance_id}:`, err);
              failCount++;
            }
          }
          this.$q.notify({
  ...
  created() {
    this.url_api = getRestApiUrl(this.$store) + "/importance";
  },
};
</script>
