...
export default {
  name: "CareerFormSuper",
  data() {
    return {
      career_: ref(null),
      file_export: "",
      url_api: "",
      title: "อาชีพ(ผู้ดูแลกลุ่ม)",
  ...
  methods: {
    ...
    async exportTable() {
      ...
      try {
        ...
        mainTitle.value = `รายงานข้อมูลอาชีพ (Super User Constance) - ${new Date().toLocaleDateString('th-TH')}`;
        mainTitle.font = { name: 'Sarabun', size: 16, bold: true };
        mainTitle.alignment = { horizontal: 'center', vertical: 'middle' };
        worksheet.getRow(1).height = 40;

        const headerRow = worksheet.getRow(2);
        headerRow.values = ['รหัส', 'กลุ่มอาชีพ', 'อาชีพ'];
        headerRow.height = 30;
    ...
    submitForm() {
      ...
      if (!this.isEdit) {
        ...
        this.$q.dialog({
          ...
          axios.put(`${this.url_api}/${this.individual.career_id}`, data).then((response) => {
        ...
        try {
          for (const item of this.selected) {
            try {
              ...
              console.error(`Failed to delete ID ${item.career_id}:`, err);
              failCount++;
            }
          }
          this.$q.notify({
    ...
    getCareer_group() {
      axios.get(getRestApiUrl(this.$store) + "/career-groups").then((res) => {
