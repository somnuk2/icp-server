...
test.describe('Integrated Flow: FormComponent (Real DB)', () => {
  ...
  test('FormComponent: Fill -> Graduate Year -> Disability -> PDPA -> Save -> Delete', async ({ page }) => {
    ...
    await test.step('Step 13: Add new Qualification (2-Step Process) + Reload persists', async () => {
      ...
      // Handle intermediate confirmation dialog for master record
      await confirmDialog(page, { timeout: T.long });
      const masterNotif = await waitForNotificationText(page, T.xlong);
      expect(masterNotif).toMatch(/เพิ่มคุณสมบัติใหม่สำเร็จ|สำเร็จ/i);
      await waitForQuasarIdle(page, T.short);

      // Verify the new qualification is selected in the field
      const qualField = qualInput.locator('xpath=ancestor::*[contains(@class,"q-field")]').first();
      await expect(qualField).toContainText(createdQualName, { timeout: T.long });

      // 4) เลือกค่าเป้าหมาย + ระดับความสำคัญ
      const target = dialog.locator('[data-testid="select-target-value"]').first();
      await target.click({ force: true });
      await page.locator('.q-menu:visible .q-item').first().click({ force: true });

      const level = dialog.locator('[data-testid="select-level"]').first();
      ...
      // Final confirm if any (FormQualification uses reset/resetForm which closes dialog after notify)
      // Confirm logic depends on if there's a second confirm dialog. Based on FormQualification.vue,
      // submitForm doesn't have a second confirm dialog, it just notifies and resets.

      const finalNotif = await waitForNotificationText(page, T.xlong);
      expect(finalNotif).toMatch(/บันทึกข้อมูลสำเร็จ|สำเร็จ/i);
