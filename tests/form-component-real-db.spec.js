...
test.describe('Integrated Flow: FormComponent (Real DB)', () => {
  ...
  test('FormComponent: Fill -> Graduate Year -> Disability -> PDPA -> Save -> Delete', async ({ page }) => {
    ...
    // await test.step('Step 13: Add new Qualification (2-Step Process) + Reload persists', async () => {
    //   const tree = page.locator('.q-tree').first();
    //   await expect(tree).toBeVisible({ timeout: T.xlong });

    //   // Open "เพิ่มข้อมูลด้วยตนเอง" dialog
    //   const addBtn = page.getByRole('button', { name: 'เพิ่มข้อมูลด้วยตนเอง' }).first();
    //   await addBtn.click({ force: true });

    //   // Stable Dialog Locator: Filter by title to avoid shifting issues
    //   const dialog = page.locator('.q-dialog:visible')
    //     .filter({ hasText: /เพิ่มข้อมูลคุณสมบัติ|แก้ไขข้อมูล/ })
    //     .last();
    //   await expect(dialog).toBeVisible({ timeout: T.long });

    //   // 1) เลือก career (existing)
    //   const careerInput = dialog.getByTestId('select-target-career');
    //   const pickedCareer = await pickFirstFromMenu(page, careerInput, T.xlong);

    //   // 2) เลือก group (existing) - เลือกก่อนเพื่อให้ Master Record ใช้กลุ่มนี้
    //   const groupInput = dialog.getByTestId('select-qualification-group');
    //   const pickedGroup = await pickFirstFromMenu(page, groupInput, T.xlong);

    //   // 3) STEP 1: Create Master Record
    //   const createdQualName = uniq('ทักษะ-IT');
    //   const qualInput = dialog.getByTestId('select-qualification-name');

    //   // [Checklist Point 1]: บันทึกคุณสมบัติ/ทักษะใหม่หรือยัง
    //   // createNewInSelect handles the Confirm dialog internally
    //   await createNewInSelect(page, qualInput, createdQualName, T.xlong, { confirmAfterCreate: true });

    //   const masterNotif = await waitForNotificationText(page, T.xlong);
    //   expect(masterNotif, 'Checklist 1: Master record should be saved').toMatch(/เพิ่มคุณสมบัติใหม่สำเร็จ|สำเร็จ/i);
    //   await waitForQuasarIdle(page, T.short);

    //   // [Checklist Point 2]: ทำการเลือก บันทึกคุณสมบัติ/ทักษะ จาก dropdown list ได้หรือยัง
    //   const qualField = qualInput.locator('xpath=ancestor::*[contains(@class,"q-field")]').first();
    //   await expect(qualField, 'Checklist 2: Qualification should be selected in field').toContainText(createdQualName, { timeout: T.long });

    //   const groupField = groupInput.locator('xpath=ancestor::*[contains(@class,"q-field")]').first();
    //   await expect(groupField, 'Group should be correctly populated for the new qualification').toContainText(pickedGroup, { timeout: T.long });

    //   // [Checklist Point 3]: เลือก ค่าเป้าหมาย จาก dropdown list ได้หรือยัง
    //   console.log('Selecting target value...');
    //   const targetInput = dialog.getByTestId('select-target-value');
    //   const pickedTarget = await pickFirstFromMenu(page, targetInput, T.xlong);
    //   expect(pickedTarget, 'Checklist 3: Target value should be selected').toBeTruthy();

    //   // [Checklist Point 4]: เลือก ความสำคัญ จาก dropdown list ได้หรือยัง
    //   console.log('Selecting importance (level)...');
    //   const levelInput = dialog.getByTestId('select-level');
    //   const pickedLevel = await pickFirstFromMenu(page, levelInput, T.xlong);
    //   expect(pickedLevel, 'Checklist 4: Importance level should be selected').toBeTruthy();

    //   // 5) STEP 2: Link to Career (Final Submit)
    //   await dialog.getByTestId('btn-submit-qual').click({ force: true });

    //   const finalNotif = await waitForNotificationText(page, T.xlong);
    //   expect(finalNotif, 'Final linkage to career should succeed').toMatch(/บันทึกข้อมูลสำเร็จ|สำเร็จ/i);

    //   // 6) Verification in Tree
    //   const careerHeader = tree.locator('.q-tree__node-header').filter({ hasText: pickedCareer }).first();
    //   await expect(careerHeader).toBeVisible({ timeout: T.xlong });

    //   if ((await careerHeader.getAttribute('aria-expanded')) === 'false') {
    //     const arrow = careerHeader.locator('.q-tree__arrow').first();
    //     if (await arrow.count()) await arrow.click({ force: true });
    //     else await careerHeader.click({ force: true });
    //     await expect(careerHeader).toHaveAttribute('aria-expanded', 'true', { timeout: T.long });
    //   }

    //   const qualHeader = tree.locator('.q-tree__node-header').filter({ hasText: createdQualName }).first();
    //   await expect.poll(async () => await qualHeader.count(), { timeout: T.xlong })
    //     .toBeGreaterThan(0);

    //   // 7) Reload and check again
    //   await page.reload({ waitUntil: 'domcontentloaded' });

    //   const tree2 = page.locator('.q-tree').first();
    //   await expect(tree2).toBeVisible({ timeout: T.xlong });
    //   await waitForQuasarIdle(page, T.xlong, { ignoreSpinners: true, ignoreInnerLoading: true });

    //   const careerHeader2 = tree2.locator('.q-tree__node-header').filter({ hasText: pickedCareer }).first();
    //   await expect(careerHeader2).toBeVisible({ timeout: T.xlong });

    //   if ((await careerHeader2.getAttribute('aria-expanded')) === 'false') {
    //     const arrowElement = careerHeader2.locator('.q-tree__arrow').first();
    //     if (await arrowElement.count()) await arrowElement.click({ force: true });
    //     else await careerHeader2.click({ force: true });
    //     await expect(careerHeader2).toHaveAttribute('aria-expanded', 'true', { timeout: T.long });
    //   }

    //   const qualHeader2 = tree2.locator('.q-tree__node-header').filter({ hasText: createdQualName }).first();
    //   await expect.poll(async () => await qualHeader2.count(), { timeout: T.xlong })
    //     .toBeGreaterThan(0);
    await test.step('Step 13: Add new Qualification (2-Step Process) + Reload persists', async () => {
      const tree = page.locator('.q-tree').first();
      await expect(tree).toBeVisible({ timeout: T.xlong });
      ...
      const fieldOf = (loc) => loc.locator('xpath=ancestor::*[contains(@class,"q-field")]').first();
      ...
      // 2) เลือก group (existing)
      const groupInput = dialog.getByTestId('select-qualification-group');
      const pickedGroup = await pickFirstFromMenu(page, groupInput, T.xlong);

      // 3) Create New Record
      createdQualName = uniq('ทักษะ-IT');
      console.log(`[Step 13] Creating new qualification: ${createdQualName}`);

      const qualInput = dialog.getByTestId('select-qualification-name');
      await createNewInSelect(page, qualInput, createdQualName, T.xlong, {
        confirmAfterCreate: true,
        addNewRegex: /เพิ่ม|สร้าง|บันทึกใหม่|เพิ่มข้อมูล|เพิ่มคุณสมบัติ/i,
      });

      // 4. แสดง notification (รอและตรวจสอบ)
      const successNotif = await waitForNotificationText(page, T.xlong);
      expect(successNotif).toMatch(/เพิ่มคุณสมบัติใหม่สำเร็จ|สำเร็จ/i);

      // 5. ทำการเลือกข้อมูล (ตรวจสอบว่าเลือกแล้วใน field)
      const qualField = fieldOf(qualInput);
      await expect(qualField, 'Qualification should be selected in field')
        .toContainText(createdQualName, { timeout: T.long });

      // 6) STEP 2: Link to Career (Final Submit)
      await dialog.getByTestId('btn-submit-qual').click({ force: true });

      const finalNotif = await waitForNotificationText(page, T.xlong);
      expect(finalNotif, 'Final linkage should succeed')
        .toMatch(/บันทึกข้อมูลสำเร็จ|สำเร็จ/i);

      // (optional) ถ้าระบบปิด dialog เองให้รอ แต่ไม่บังคับ
      ...
      // expand career ถ้ายังพับอยู่
      const expanded = await careerHeader.getAttribute('aria-expanded').catch(() => null);
      if (expanded === 'false') {
        const arrow = careerHeader.locator('.q-tree__arrow').first();
        if (await arrow.count()) await arrow.click({ force: true });
        else await careerHeader.click({ force: true });
      ...
      const expanded2 = await careerHeader2.getAttribute('aria-expanded').catch(() => null);
      if (expanded2 === 'false') {
        const arrow2 = careerHeader2.locator('.q-tree__arrow').first();
        if (await arrow2.count()) await arrow2.click({ force: true });
        else await careerHeader2.click({ force: true });
        await expect(careerHeader2).toHaveAttribute('aria-expanded', 'true', { timeout: T.long });
      }
